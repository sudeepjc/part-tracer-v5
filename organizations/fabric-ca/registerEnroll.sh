...
#  rm -rf $FABRIC_CA_CLIENT_HOME/msp
  ...
  echo
  set -x
  fabric-ca-client register --caname ca-general --id.name generaladmin --id.secret generaladminpw --id.type admin --tls.certfiles ${PWD}/organizations/fabric-ca/general/tls-cert.pem
  set +x
  ...
  echo
  set -x
	fabric-ca-client enroll -u https://generaladmin:generaladminpw@localhost:7054 --caname ca-general -M ${PWD}/organizations/peerOrganizations/general.parttracer.com/users/Admin@general.parttracer.com/msp --tls.certfiles ${PWD}/organizations/fabric-ca/general/tls-cert.pem
  set +x
...
#  rm -rf $FABRIC_CA_CLIENT_HOME/msp
  ...
  echo
  set -x
  fabric-ca-client register --caname ca-airbus --id.name airbusadmin --id.secret airbusadminpw --id.type admin --tls.certfiles ${PWD}/organizations/fabric-ca/airbus/tls-cert.pem
  set +x
  ...
  echo
  set -x
	fabric-ca-client enroll -u https://airbusadmin:airbusadminpw@localhost:8054 --caname ca-airbus -M ${PWD}/organizations/peerOrganizations/airbus.parttracer.com/users/Admin@airbus.parttracer.com/msp --tls.certfiles ${PWD}/organizations/fabric-ca/airbus/tls-cert.pem
  set +x
